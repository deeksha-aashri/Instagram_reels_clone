...
function Upload({userData}) {
  ...
  const handleChange=(e)=>{
    ...
    console.log(file);
    if(file==null){
      setError("File not selected, please select a file.");
      setTimeout(()=>{setError('')},2000);
      return;
    }
    if(file.size/(1024*1024)>sizeLimit){
     setError(`File too large. Accepts file lesser than${sizeLimit} MB`);
     setTimeout(()=>{setError('')},2000);
     return;
    }
    //If we have reached this point it means the file was of right kind and no error occured so, it has been uploaded

    let uid=uuidv4();
      setLoading(true);
      //making a new collection for the userdata to be used in posts
      ...
      uploadTask.on(
        ...
        () => {
          ...
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            ...
            console.log("post object is", postObj);
            await setDoc(doc(db, "posts", uid), postObj);
            console.log("Post added to post collection");
  
            //Update the posts array in the users db by pushing post's uid in it
